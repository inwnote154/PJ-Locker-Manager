...
<html lang="en">
  ...
  <body class="bg-light">
    ...
    <div class="d-flex">
      <div class="m-3 w-100">ที่เก็บรองเท้าบัดดี้</div>
      <div class="m-2 d-flex w-25">
        ...
        <div class=" w-50 ms-1"><a href="lockerbuddy.php"><button class="w-100 btn btn-outline-danger rounded-5" type = "button">ย้อนกลับ</button></a></div>
      </div>
    </div>
    <?php
...
if (isset($_POST['status'])){
  $status = $_POST['status'];
      ...
      if( $conn ) {
          $sql = "SELECT buddy_locker.*,building.*
          FROM buddy_locker
          left JOIN building ON buddy_locker.build_id = building.building_id
          WHERE buddy_locker.buddy_number IS NOT NULL
              ";
          ...
          elseif ($selection !== NULL){
              $sql .= " AND buddy_number LIKE '%$selection'";     
              $stmt = sqlsrv_query($conn, $sql );
          }
          if ($build !== ""){
              $sql .= " AND build_id = ?";     
          ...
          if ($stmt !== false) {
              ...
              while ($row = sqlsrv_fetch_array($stmt,SQLSRV_FETCH_ASSOC)) {
                $name='';
                  ...
                  WHERE Accesslog.StatusEm = 'active' AND Accesslog.LockerNumber = '$row[buddy_number]' AND Accesslog.TypeLocker = 'buddy'";
