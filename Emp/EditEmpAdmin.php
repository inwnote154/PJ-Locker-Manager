                ...
                <div class="container-fluid">
                    <?php
                      if( $conn ) {
                        ...
                        $sql3 = "SELECT * FROM lineproduction ";
                                ...
                                <?php
                                    if ($stmt !== false) {
                                      ...
                                      while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                          echo "<form action='../Emp/adminEditer.php' method='post'>";
                                          echo "<tr><td class=' p-2 pb-4'>รหัสพนักงาน</td>
                                          
                                          <td class='w-30 p-2 pb-4'><input class='form-control form-control-sm' type='text' id='employee_id' name='employee_id' value='". $row['idcard'] ."'readonly>  </td>";
                              
                                          echo "<td class=' p-2 pb-4'>คำนำหน้า</td>
                                          <td class='w-30 p-2 pb-4'>
                                          <select class='form-control form-control-sm' id='employee_in' name='employee_in'>
                                              <option selected value='" . $row['InitialT'] . "'>" . $row['InitialT'] . "</option>
                                              <option value='นาย'>นาย</option>
                                              <option value='นาง'>นาง</option>
                                              <option value='นางสาว'>นางสาว</option>
                                          </select>
                                          </td></tr><tr>";
                                          echo "<td class=' p-2 pb-4'>ชื่อ</td>
                                          <td class=' p-2 pb-4'><input class='form-control form-control-sm' type='text' id='employee_name' name='employee_name' value='" . $row['namethai'] . "'></td>";
                                          echo "<td class=' p-2 pb-4'>นามสกุล</td>
                                          <td class=' p-2 pb-4'><input class='form-control form-control-sm' type='text' id='employee_sur' name='employee_sur' value='" . $row['surnamethai'] . "'></td></tr>";
                                          echo "<td class=' p-2 pb-4'>แผนก</td>
                                          <td class=' p-2 pb-4'>
                                          <select class='form-control form-control-sm' id='employee_depart' name='employee_depart'>
                                          <option selected value='" . $row['departmentno'] . "'>" . $row['departmentno'] ." ".$row['departmentname']."</option>";
                                          while($row2 = sqlsrv_fetch_array($stmt2, SQLSRV_FETCH_ASSOC)){
                                              echo "
                                              <option value='".$row2['departmentno']."'>".$row2['departmentno'] ." ".$row2['departmentname']."</option>";
                                          }
                                          echo "</select>
                                          </td>";
                                          echo "<td class=' p-2 pb-4'>ไลน์ผลิต</td>
                                          <td class=' p-2 pb-4'>
                                          <select class='form-control form-control-sm' id='employee_line' name='employee_line' value='" . $row['lineid'] . "'>
                                          <option selected value='" . $row['lineid'] . "'>" . $row['linename'] . "</option>";
                                          while($row3 = sqlsrv_fetch_array($stmt3, SQLSRV_FETCH_ASSOC)){
                                              echo "
                                              <option value='".$row3['lineid']."'>".$row3['linename']."</option>";
                                          }
                                          echo "</select>
                                          </td></tr></table>
                                          <input type='hidden' name='departname' id='admin' value='".$row['departmentname']."' readonly>";
                              
                                          echo '<center>
                                            <button type="submit" href="#" class="btn btn-success btn-icon-split mr-1">
                                                <span class="icon text-white-50">
                                                    <i class="fas fa-check"></i>
                                                </span>
                                                <span class="text">ยืนยัน</span>
                                            </button>';
                                          echo '<a href="javascript:history.back()" class="btn btn-danger btn-icon-split">
                                          <span class="icon text-white-50">
