...
<html lang="en">
  ...
  <body class="bg-light">
    ...
    <form class="d-flex" action="lockershirt.php" method="post">
      <div class="col" >
          <div class="d-flex">
            <?php
            // $admin = $_GET['admin'];
    ...
    <div class="d-flex">
      ...
      <div class="m-2 d-flex w-25">
        ...
        <div class=" w-50 ms-1"><a href="lockershirt.php"><button class="w-100 btn btn-outline-danger rounded-5" type = "button">ย้อนกลับ</button></a></div>
      </div>
    </div>
    <?php
...
if (isset($_POST['status'])){
  $status = $_POST['status'];
      ...
      if( $conn ) {
          $sql = "SELECT locker_shirt.*,building.*
          FROM locker_shirt
          left JOIN building ON locker_shirt.build_id = building.building_id
          WHERE locker_shirt.shirt_number IS NOT NULL
              ";
          ...
          elseif ($selection !== NULL){
              $sql .= " AND shirt_number LIKE '%$selection'";     
              $stmt = sqlsrv_query($conn, $sql );
          }
          if ($build !== ""){
              $sql .= " AND build_id = ?";     
          ...
          if ($stmt !== false) {
              ...
              while ($row = sqlsrv_fetch_array($stmt,SQLSRV_FETCH_ASSOC)) {
                $name='';
                  ...
                  WHERE Accesslog.StatusEm = 'active' AND Accesslog.LockerNumber = '$row[shirt_number]' AND Accesslog.TypeLocker = 'shirt'";
